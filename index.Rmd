...
output:
  xaringan::moon_reader:
    lib_dir: libs
    css: ["default", "assets/css/fighton-fonts.css", "assets/css/fighton-theme2.css", "assets/css/fighton-theme.css"]
...
![:scale 50%](images/2-Line_KeckSOMofUSC_CardOnBlack.jpg)
]
...
## CLINICAL ADHD versus COGNITIVE ADHD
...
nbhv1 <- neurobehav %>%
  ...
  dplyr::summarize(zMean = mean(z),
                   zPct = mean(percentile)) %>%
  dplyr::mutate(range = NA)
nbhv1$zMean <- round(nbhv1$zMean, 2L)
...
nbhv_level2_drill <-
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    # round z-score to 1 decimal
    nbhv2$zMean <- round(nbhv2$zMean, 2L)
...
nbhv_level3_drill <-
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    lapply(unique(nbhv2$subdomain), function(y_level) {
      ...
      # round z-score to 1 decimal
      nbhv3$zMean <- round(nbhv3$zMean, 2L)
  ...
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    lapply(unique(nbhv2$subdomain), function(y_level) {
      ...
      lapply(unique(nbhv3$narrow), function(z_level) {
        ...
        # round z-score to 1 decimal
        nbhv4$zMean <- round(nbhv4$zMean, 2L)
...
plot1 <-
  ...
  hc_add_series(
    nbhv_level1_status,
    type = "bar",
    name = "Neurobehavioral Rating Scale Scores",
    hcaes(x = name, y = y)) %>%
  hc_xAxis(
    type = "category",
    title = list(
